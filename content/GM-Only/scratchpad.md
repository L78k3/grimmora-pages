...
```dataviewjs
// --- CONFIGURATION ---
// Enter the folder path you want to search in (e.g., "Projects/Active")
...
function collectLinks(linkCache) {
    for (let sourceFile in linkCache) {
        // Only proceed if the source file is inside the target folder
        if (sourceFile.startsWith(targetFolder)) {
            const linksInFile = linkCache[sourceFile];
            for (let targetLink in linksInFile) {
...
collectLinks(resolved);
collectLinks(unresolved);

dv.header(2, `Wikilinks found in "${targetFolder}"`);
dv.paragraph(`Total Unique Links: ${allLinks.size}`);
dv.list(Array.from(allLinks).sort());
```
